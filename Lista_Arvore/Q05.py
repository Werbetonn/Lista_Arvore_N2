...
class ArvoreBinaria:
    ...
    def travessia_inordem(self):
        if self.raiz is None:
            ...
            return self.travessia_inordem_recursivo(self.raiz)

    def travessia_inordem_recursivo(self, no):
        valores = []
        if no.esquerda is not None:
            valores += self.travessia_inordem_recursivo(no.esquerda)
        valores.append(no.valor)
        if no.direita is not None:
            valores += self.travessia_inordem_recursivo(no.direita)
        return valores
...
travessia = arvore.travessia_inordem()
print(f"Travessia inordem: {travessia}")
