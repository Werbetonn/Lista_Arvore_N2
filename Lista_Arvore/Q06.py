...
class ArvoreBinaria:
    ...
    def travessia_pre_ordem(self):
        if self.raiz is None:
            ...
            return self.travessia_pre_ordem_recursivo(self.raiz)

    def travessia_pre_ordem_recursivo(self, no):
        valores = [no.valor]
        if no.esquerda is not None:
            valores += self.travessia_pre_ordem_recursivo(no.esquerda)
        if no.direita is not None:
            valores += self.travessia_pre_ordem_recursivo(no.direita)
        return valores
...
travessia_pre_ordem = arvore.travessia_pre_ordem()
print(f"\nTravessia pré-ordem: {travessia_pre_ordem}")
