...
class ArvoreBinaria:
    ...
    def calcular_altura(self):
        return self.calcular_altura_recursivo(self.raiz)

    def calcular_altura_recursivo(self, no):
        if no is None:
            return -1
        altura_esquerda = self.calcular_altura_recursivo(no.esquerda)
        altura_direita = self.calcular_altura_recursivo(no.direita)
